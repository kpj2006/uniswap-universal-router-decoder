...
web3_provider = os.environ.get('QUICKNODE_BSC_MAINNET', 'https://binance.llamarpc.com/')
...
def send_transaction(value, encoded_data):
    ...
    raw_transaction = account.sign_transaction(trx_params).raw_transaction
...
def buy_usdt():
    ...
    assert usdt_balance > 0, f"USDT balance should be > 0, was actually: {usdt_balance}"
...
def sell_usdt_part_1():
    # Get actual USDT balance and use half of it
    usdt_balance = usdt_contract.functions.balanceOf(account.address).call()
    amount_in = usdt_balance // 2  # Use half of available USDT
    amount_out_min = 0
    ...
    print(f"Using {amount_in / 10**18:.6f} USDT for part 1")
    ...
    assert usdt_balance > 0, f"USDT balance should be > 0, was actually: {usdt_balance}"
    assert wbnb_balance > 0, f"WBNB balance should be > 0, was actually: {wbnb_balance}"

    print(" => SELL USDT for WBNB PART 1: OK")


def sell_usdt_part_2():
    # Get actual USDT balance and use the remaining amount
    usdt_balance = usdt_contract.functions.balanceOf(account.address).call()
    amount_in = usdt_balance  # Use all remaining USDT
    amount_out_min = 0
    v3_path = [usdt_address, 500, wbnb_address]
    ...
    print(f"Using {amount_in / 10**18:.6f} USDT for part 2")
